...
/**
 * GitCode 文本生成适配器
 * 使用 GitCode 的 DeepSeek 模型
 */
class GitCodeAdapter extends BaseLLMAdapter {
  constructor(config) {
    super(config);
    this.name = 'gitcode';
  ...
  async testConnection() {
    try {
      ...
      console.error('GitCode connection test failed:', error.message);
      return false;
    }
  }
}

module.exports = GitCodeAdapter;
