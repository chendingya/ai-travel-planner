...
app.post('/api/generate-share-content', async (req, res) => {
  ...
  try {
    ...
    console.log(`📝 正在为【${destination}】生成【${platform}】平台【${emotion}】风格的分享文案...`);

    // 构建行程摘要
    const itinerarySummary = dailyItinerary.map((day, index) => {
      const activities = (day.activities || []).slice(0, 3).map(a => a.description || a.location).filter(Boolean);
...
${emotion === '种草' ? '风格：充满热情，强调必去、绝不能错过、yyds、真的绝'
  : emotion === '治愈' ? '风格：温暖治愈，强调慢生活、放松、疗愈、静享'
  : emotion === '攻略' ? '风格：干货实用，强调省钱、避雷、攻略、建议收藏'
  : emotion === 'emo' ? '风格：文艺emo，强调情感、感受、治愈、回忆'
...
${emotion === '种草' ? '强调此行的新发现、惊喜和值得推荐'
  : emotion === '治愈' ? '强调放松、内心感受、小美好'
  : emotion === '攻略' ? '强调收获和学到的东西'
  : emotion === 'emo' ? '强调情绪感受、人生感悟'
  : '强调趣事、糗事或有趣的经历'}
...
${emotion === '种草' ? '风格：激情种草，强调"你必须来"、"绝了"'
  : emotion === '治愈' ? '风格：治愈向，强调舒适感、视觉美'
  : emotion === '攻略' ? '风格：干货分享，强调实用信息和技巧'
  : emotion === 'emo' ? '风格：文艺向，强调情感和意境'
  : '风格：娱乐搞笑，强调趣味性和参与感'}
...
重点打卡地：${highlights.join('、')}
    ...
    const userPrompt = `请为以下旅行计划生成${platformPrompt.name}风格的分享文案：
...
app.post('/api/generate-playlist', async (req, res) => {
  ...
  try {
    ...
    console.log(`🎵 正在为【${destination}】【${style}】风格生成旅途BGM歌单...`);

    const highlightStr = highlights && highlights.length > 0 ? highlights.join('、') : '城市风景';
...
${presetStyles[style] ? `风格特征：${styleDescription}` : `自定义风格"${style}"：请根据此风格特点选择合适的音乐类型和氛围`}

请生成符合目的地文化特色和旅行风格的音乐歌单。如果是国内城市，优先选择本地音乐或与城市相关的经典歌曲。例如长沙可以包括本地说唱、湘江相关音乐等。

${!presetStyles[style] ? `对于"${style}"这个自定义风格，请选择最能体现这种旅行氛围的音乐类型。` : ''}
