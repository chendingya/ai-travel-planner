...
graph TB
    subgraph Client["客户端层"]
        FE[Frontend<br/>Vue.js 应用]
    end

    subgraph Controller["控制层 (Controller Layer)"]
        APIRouter[API Routes<br/>路由聚合]
    ...
    subgraph Service["服务层 (Service Layer)"]
        AIChatSvc[AIChatService<br/>AI 聊天与工具调用]
        PlanSvc[PlanService<br/>旅行规划]
        ImageSvc[ImageService<br/>图片生成]
        MCPSvc[MCPService<br/>MCP 工具管理]
        OtherSvc[Other Services<br/>TTS/Prompt/Playlist/...]
    end

    subgraph LangChain["LangChain 集成层"]
        LCMgr[LangChainManager<br/>统一 AI 调用管理]
        TextAdapters[Text Adapters<br/>OpenAICompatible]
        ImageAdapters[Image Adapters<br/>ModelScope]
    end

    subgraph AIProviders["AI 提供商层"]
    ...
    subgraph MCPTools["MCP 工具层"]
        Tool12306[12306<br/>火车票查询]
        ToolAmap[Amap<br/>高德地图]
        ToolBing[Bing<br/>网络搜索]
        OtherMCP[Other MCP Servers]
    end

    subgraph Data["数据层 (Data Layer)"]
        Supabase[Supabase<br/>PostgreSQL]
        LocalFS[Local File System<br/>音频/图片存储]
    end
