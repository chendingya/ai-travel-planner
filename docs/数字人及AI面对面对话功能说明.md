...
*   **沉浸式聊天界面**：
    ...
    *   背景采用渐变色，营造轻松的对话氛围。
*   **语音交互**：
    *   **文字+语音回复**：AI在回复文字的同时，会同步生成语音，模拟面对面交流。
    *   **实时生成**：利用流式或快速响应技术，减少等待时间。
    *   **长文本分段**：针对AI生成的长篇回复（如详细的行程规划），系统会自动将其切分为多段音频，按顺序播放，突破TTS服务的长度限制。
*   **个性化设置**：
    *   **音色定制**：用户可以在界面顶部随时切换AI的音色。
    *   **自动播放**：可控制收到消息后是否自动朗读。
*   **快捷提问**：
...
*   **前端视图**：`frontend/src/views/AIChatView.vue`
*   **音频处理**：
    *   前端实现了复杂的音频播放逻辑，支持单段和多段音频的队列播放。
    *   通过轮询机制 (`pollAudioStatus`) 处理异步生成的音频任务。
*   **后端逻辑** (`tts-flask-backend/main.py`)：
    *   **LLM集成**：使用 ModelScope (DeepSeek-V3.2-Exp) 生成高质量的文本回复。
    *   **TTS集成**：使用 DashScope (qwen3-tts-flash) 进行语音合成。
    *   **智能分段**：后端实现了智能分段算法，根据标点符号和字节长度限制，将长文本切分为适合TTS处理的小段，解决了长文本语音合成的问题。
