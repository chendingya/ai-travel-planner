...
*   **智能问答**：
    *   基于大语言模型，能够回答关于旅游的各种问题，包括路线规划、交通指南、避坑建议等。

### 2.3 近期交互优化（2026-03-05）

*   **工具调用展示优化**：
    *   新增前端独立流事件解析器 `aiStreamEventParser.js`，将工具调用/返回/异常与正文消息分离展示。
    *   默认只显示工具摘要，避免直接暴露大段 JSON 结构。
    *   支持调试参数 `?debug_tool_raw=1` 查看原文片段。
*   **历史侧栏结构优化**：
    *   历史记录改为独立侧边栏，内部滚动，不再随页面内容长度无限拉伸。
    *   侧栏高度跟随屏幕视口，聊天主区域与历史区域滚动解耦。


### 2.2 使用场景

*   用户在出行前需要制定详细计划，或者在旅途中遇到具体问题（如"现在去西湖人多吗？"）时，可以通过此功能获得即时、专业的解答。
