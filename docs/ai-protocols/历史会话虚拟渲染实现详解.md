...
### 3.4 一个具体例子
假设：
- `historyListHeight = 420`
- `historyItemHeight = 84`
- `historyOverscan = 6`
- `displayedSessions.length = 2000`
- 当前滚动位置 `historyScrollTop = 840`

则：
1. 可视区大约能容纳 `420 / 84 = 5` 条。
2. 加上上下各 `6` 条 overscan：  
`visibleCount = ceil(420 / 84) + 2 * 6 = 5 + 12 = 17`
3. 当前滚动到约第 `840 / 84 = 10` 条附近：  
`start = max(0, floor(840 / 84) - 6) = max(0, 10 - 6) = 4`
4. 结束索引：  
`end = min(2000, 4 + 17) = 21`
5. 实际渲染的数据窗口：  
`visibleSessions = displayedSessions.slice(4, 21)`

这表示：
- 实际只渲染第 `4` 到第 `20` 条会话
- 顶部占位高度：`paddingTop = 4 * 84 = 336px`
- 底部占位高度：`paddingBottom = (2000 - 21) * 84 = 166236px`

因此浏览器滚动条看起来仍像在滚动 2000 条完整数据，但真实 DOM 中只有 17 条会话项，加上上下两个占位块。这样既保留了连续滚动体验，也显著减少了渲染节点数量。
...
1. 为每条新增记录写入 `delayMap[id] = min(index * 55ms, 500ms)`，避免长列表出现过长等待
2. 列表项带上 `is-appearing` 类和 `--appear-delay`
3. CSS `historyItemAppear` 动画执行（位移+透明度）
4. 用户手动滚动历史列表时，会清空剩余延迟，确保滚到目标位置后卡片立即可见
5. 定时清理延迟 map，避免重复滚动时重复触发
