...
- `middleware/auth.js`
  ...
  - 鉴权成功后会读取当前用户的 provider 配置，并注入本次请求的运行时上下文。
...
- 其他业务服务
  ...
  - `providerConfigService`：提供商配置管理（脱敏读取、密钥保留/替换、连通性校验、Supabase 持久化、请求级运行时加载）。

### 5.4 AI 与外部能力层
- `LangChainManager`
  - 统一管理文本/图像/Embedding/Rerank provider 适配器。
  - 提供 provider fallback、错误探测、createAgent 能力。
  - 启动时加载默认 provider，并支持按请求覆盖当前用户的 provider 上下文。
...
- 配置优先级：登录用户请求时优先加载其在 Supabase 的个人 provider 配置，缺失时回退 `.env`；保存成功后从该用户后续请求开始立即生效。
- 配置隔离：provider 配置按请求隔离，不再通过进程级全局热更新覆盖其他用户的运行时。
