...
- TTS（Piper，可选）：`TTS_PROVIDER=piper`、`TTS_PIPER_MODEL=/app/runtime/piper/model.onnx`、`TTS_PIPER_CONFIG=/app/runtime/piper/model.onnx.json`，可配 `TTS_PIPER_MODEL_URL` / `TTS_PIPER_CONFIG_URL` 在容器启动时自动下载模型
